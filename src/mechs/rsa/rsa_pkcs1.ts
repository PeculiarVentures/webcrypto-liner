...
export class RsaPkcs1Provider extends core.ProviderCrypto {
  ...
  public async onSign(algorithm: RsaPkcs1SignParams, key: RsaCryptoKey, data: ArrayBuffer): Promise<ArrayBuffer> {
    RsaCrypto.checkLib();

    
    new asmCrypto.RSA(key.data);
  ...
  public async onVerify(algorithm: RsaPkcs1SignParams, key: RsaCryptoKey, signature: ArrayBuffer, data: ArrayBuffer): Promise<boolean> {
    const ok = crypto
  ...
  public checkVerify(algorithm: RsaPkcs1SignParams, key: RsaCryptoKey, signature: ArrayBuffer, data: ArrayBuffer) {
    this.checkAlgorithmName(algorithm);
    this.checkAlgorithmSignParams(algorithm);
